...
Status: revised after critic review — pending implementation.
Date: 2026-09-05 (revised from 2026-07-09 original)
...
| 4 | Auth | Tailnet-only bind + 6-hex pairing code printed by `/rc`, entered once in the app. Rate-limit failed `hello`: 5 per IP → 60 s lockout. Code stored in `CodeSettings` (SettingsManager), not Keychain — tailnet membership is the real boundary |
...
3. **Event names**: The spec lists specific event names (`message_start`,
...
4. **History API**: The spec references `sessionManager.buildContextEntries()`.
...
5. **WebSocket server dependency**: The spec says "stdlib only" with `node:ws`, but
...
subscribes to all discoverable events, and attempts `sendUserMessage`. Run it, capture
the output, then delete the probe and update this spec with findings before proceeding
to Task 1.
...
- `pi-extensions/rc/index.ts` — extension factory:
  - `pi.registerCommand("rc")` — **toggle** (no arguments):
    - If server is off → start it:
      - Get Tailscale IPv4 (`tailscale ip -4`), bind `node:http` + WS upgrade on
        `ws://<ip>:47800` (plain WebSocket, no TLS).
      - Generate 6-hex code on first use, persist in `~/.pi/agent/rc.json` (also
        holds the port).
...
- The WS server + client set + pending-question registry live on `globalThis`
  (guarded key, e.g. `globalThis.__piRc`), created lazily by `/rc` toggle-on.
...
```
     ...
     pi starts ──→│  STOPPED    │←──── /rc (when ON)
                  ...
                  │  (same svr) │  - detect globalThis.__piRc
...
|------|---------|
| `hello_ok` | `{version: 1}` |
| `state` | `{cwd, sessionName, sessionId, model: {provider, id}, thinkingLevel, isStreaming, contextUsage?}` |
| `history` | `{messages: [...], cursor?}` — see History format below |
| `event` | `{name, ...}` forwarded pi events |
...
type HistoryMessage =
  ...
  | { role: "assistant"; content: ContentBlock[] }  // text, thinking, toolUse
...
`get_history {cursor}` (added to client→server protocol). In practice, sessions
rarely exceed 200 entries before compaction.
...
**Modification**: At the top of `execute()`, before the `ctx.mode !== "tui"` check,
attempt remote-aware ask:

```typescript
// If rc server is running, try remote ask first
const rcAsk = globalThis.__piRc?.ask;
if (rcAsk) {
  const result = await rcAsk({ id: uuidv7(), kind: "question", params });
  if (result !== null) {
    // Remote client answered
    return { content: [{type: "text", text: `User selected: ${result.value}`}], details };
  }
  // null = no remote clients connected or all disconnected before answering → fall through to TUI
}
```

The `ask` function is exposed by the rc singleton on `globalThis.__piRc`:
- Broadcasts `question`/`questionnaire` to all connected clients
- Returns a `Promise` that resolves with the answer (first client wins) or `null`
  (all clients disconnected / `/rc` toggled off / pi exit)
- Pending asks are stored in the singleton's registry; re-delivered to new/reconnecting clients

No event bus needed — the question extensions access the rc singleton directly via
`globalThis.__piRc`. This avoids the untested `pi.events` bus sharing assumption.
...
- **Ping/pong**: Client sends `ping` every 30s via a background `Task`. Tracks last
...
- **Storage**: `CodeSettings` in `SettingsManager`: `codeHost` (String),
  `codePort` (Int, default 47800), `codePairingCode` (String). All in UserDefaults.
  No Keychain — tailnet membership is the real auth boundary; physical device access
  to read UserDefaults is a higher bar than the code itself protects against.
...
- `CodeView` (root): switches on state. Connect screen = host/port/code fields +
  Connect button; code field 6 chars, hex, uppercase. Pre-fill from `CodeSettings`
...
- `CodeViewModelTests`: connect/disconnect transitions, prompt vs steer routing by
  ...
  into view model items, reconnect re-hello.
- `CodeServerClientTests`: message framing, bad-code error mapping, re-delivered
  question handling, ping/pong — with a `MockWebSocketTransport` (protocol boundary)
  that replays canned server messages.
...
- Log all enumerable properties of `pi` (the `ExtensionAPI` object)
- Log all enumerable properties of `ctx` (the `ExtensionContext` object)
...
- Run pi, execute `/probe`, capture output
...
12. Background app → wait 2 min → foreground → verify reconnect
...
3. **Task 2 — forked question extensions + remote-aware ask**: Copy
   ...
   `pi-extensions/{question,questionnaire}/`, add `globalThis.__piRc?.ask`
   integration, update symlinks. Run test script (C2, test 5). Commit.
...
Per AGENTS.md "Change Completion": compile and commit after each completed task
(linter and formatter run on commit hook); never push to remote.

## Open items / risks

- **`sendUserMessage` API** (CRITICAL): If the probe reveals no such API, the prompt/steer/abort
  protocol must be redesigned around `ctx.ui.setEditorText()` + simulated submit, or
  a different mechanism. This is the single largest implementation risk.
- `contextUsage` availability: `ctx.getContextUsage()` may be null right after
  compaction — treat as optional in UI.
- Tailscale IPv4 lookup: `tailscale ip -4` must be on PATH in the pi process env;
  fail with a clear message (safety decision, no fallback to `0.0.0.0`).
- 6-hex code = 24 bits: acceptable as a tailnet pairing code with rate-limiting.
  Rotates per machine, not per session. Document that tailnet membership is the real
  boundary.
- App background behavior: WS drops in background; no remote push notifications in
  v1. Reconnect on foreground (standard `scenePhase` handling).
